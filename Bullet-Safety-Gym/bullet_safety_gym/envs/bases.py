...
class Agent(Body):
    ...
    @property
    def velocity_violation_red(self) -> bool:
        """ Check if agents exceeds speed constraint."""
        coef = 0.98
        vel = np.linalg.norm(self.get_linear_velocity()[:2])
        vel_constraint = True if vel > self.velocity_constraint * coef else False
        return vel_constraint
    
    @property
    def velocity_violation_x(self) -> bool:
        """ Check if agents exceeds speed constraint."""
        coef = 0.7
        vel = np.linalg.norm(self.get_linear_velocity()[0])
        vel_constraint = True if vel > self.velocity_constraint * coef else False
        return vel_constraint
    
    @property
    def velocity_violation_y(self) -> bool:
        """ Check if agents exceeds speed constraint."""
        coef = 0.7
        vel = np.linalg.norm(self.get_linear_velocity()[1])
        vel_constraint = True if vel > self.velocity_constraint * coef else False
        return vel_constraint

    @property
    def velocity_violation_low_1(self) -> bool:
        """ Check if agents exceeds speed constraint."""
        coef = 0.8
        vel = np.linalg.norm(self.get_linear_velocity()[:2])
        vel_constraint = True if vel < self.velocity_constraint * coef else False
    ...
    @property
    def velocity_violation_low_2(self) -> bool:
        """ Check if agents exceeds speed constraint."""
        coef = 0.85
        vel = np.linalg.norm(self.get_linear_velocity()[:2])
        vel_constraint = True if vel < self.velocity_constraint * coef else False
    ...
    @property
    def velocity_violation_low_3(self) -> bool:
        """ Check if agents exceeds speed constraint."""
        coef = 0.98
        vel = np.linalg.norm(self.get_linear_velocity()[:2])
        vel_constraint = True if vel < self.velocity_constraint * coef else False
