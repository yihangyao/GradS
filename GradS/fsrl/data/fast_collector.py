...
class FastCollector(object):
    ...
    def collect(
        ...
        while True:
            ...
            sub_cost_0 = self.data.info.get("sub_cost_0", np.zeros(rew.shape))
            sub_cost_1 = self.data.info.get("sub_cost_1", np.zeros(rew.shape))
            sub_cost_2 = self.data.info.get("sub_cost_2", np.zeros(rew.shape))
            sub_cost_3 = self.data.info.get("sub_cost_3", np.zeros(rew.shape))
            sub_cost_4 = self.data.info.get("sub_cost_4", np.zeros(rew.shape))
            sub_cost_5 = self.data.info.get("sub_cost_5", np.zeros(rew.shape))
            sub_cost_6 = self.data.info.get("sub_cost_6", np.zeros(rew.shape))
            sub_cost_7 = self.data.info.get("sub_cost_7", np.zeros(rew.shape))
            sub_cost_8 = self.data.info.get("sub_cost_8", np.zeros(rew.shape))
            sub_cost_9 = self.data.info.get("sub_cost_9", np.zeros(rew.shape))
            sub_cost_10 = self.data.info.get("sub_cost_10", np.zeros(rew.shape))

            sub_cost_11 = self.data.info.get("sub_cost_11", np.zeros(rew.shape))
            sub_cost_12 = self.data.info.get("sub_cost_12", np.zeros(rew.shape))
            sub_cost_13 = self.data.info.get("sub_cost_13", np.zeros(rew.shape))
            sub_cost_14 = self.data.info.get("sub_cost_14", np.zeros(rew.shape))
            sub_cost_15 = self.data.info.get("sub_cost_15", np.zeros(rew.shape))
            sub_cost_16 = self.data.info.get("sub_cost_16", np.zeros(rew.shape))
            sub_cost_17 = self.data.info.get("sub_cost_17", np.zeros(rew.shape))
            sub_cost_18 = self.data.info.get("sub_cost_18", np.zeros(rew.shape))
            sub_cost_19 = self.data.info.get("sub_cost_19", np.zeros(rew.shape))
            sub_cost_20 = self.data.info.get("sub_cost_20", np.zeros(rew.shape))

            # m_cost = self.data.info.get("m_cost", np.zeros(rew.shape, 2))
            total_cost += np.sum(cost)
            total_sub_cost_0 += np.sum(sub_cost_0)
            total_sub_cost_1 += np.sum(sub_cost_1)
            total_sub_cost_2 += np.sum(sub_cost_2)
            total_sub_cost_3 += np.sum(sub_cost_3)
            total_sub_cost_4 += np.sum(sub_cost_4)
            total_sub_cost_5 += np.sum(sub_cost_5)
            total_sub_cost_6 += np.sum(sub_cost_6)
            total_sub_cost_7 += np.sum(sub_cost_7)
            total_sub_cost_8 += np.sum(sub_cost_8)
            total_sub_cost_9 += np.sum(sub_cost_9)
            total_sub_cost_10 += np.sum(sub_cost_10)

            total_sub_cost_11 += np.sum(sub_cost_11)
            total_sub_cost_12 += np.sum(sub_cost_12)
            total_sub_cost_13 += np.sum(sub_cost_13)
            total_sub_cost_14 += np.sum(sub_cost_14)
            total_sub_cost_15 += np.sum(sub_cost_15)
            total_sub_cost_16 += np.sum(sub_cost_16)
            total_sub_cost_17 += np.sum(sub_cost_17)
            total_sub_cost_18 += np.sum(sub_cost_18)
            total_sub_cost_19 += np.sum(sub_cost_19)
            total_sub_cost_20 += np.sum(sub_cost_20)
        ...
        return {
            ...
            "sub_cost_0": total_sub_cost_0 / episode_count,
            "sub_cost_1": total_sub_cost_1 / episode_count,
            "sub_cost_2": total_sub_cost_2 / episode_count,
            "sub_cost_3": total_sub_cost_3 / episode_count,
            "sub_cost_4": total_sub_cost_4 / episode_count,
            "sub_cost_5": total_sub_cost_5 / episode_count,
            "sub_cost_6": total_sub_cost_6 / episode_count,
            "sub_cost_7": total_sub_cost_7 / episode_count,
            "sub_cost_8": total_sub_cost_8 / episode_count,
            "sub_cost_9": total_sub_cost_9 / episode_count,
            "sub_cost_10": total_sub_cost_10 / episode_count,

            "sub_cost_11": total_sub_cost_11 / episode_count,
            "sub_cost_12": total_sub_cost_12 / episode_count,
            "sub_cost_13": total_sub_cost_13 / episode_count,
            "sub_cost_14": total_sub_cost_14 / episode_count,
            "sub_cost_15": total_sub_cost_15 / episode_count,
            "sub_cost_16": total_sub_cost_16 / episode_count,
            "sub_cost_17": total_sub_cost_17 / episode_count,
            "sub_cost_18": total_sub_cost_18 / episode_count,
            "sub_cost_19": total_sub_cost_19 / episode_count,
            "sub_cost_20": total_sub_cost_20 / episode_count,


            "m_cost": [total_sub_cost_0 / episode_count, total_sub_cost_1 / episode_count, total_sub_cost_2 / episode_count,
                       total_sub_cost_3 / episode_count, total_sub_cost_4 / episode_count, total_sub_cost_5 / episode_count,
                       total_sub_cost_6 / episode_count, total_sub_cost_7 / episode_count, total_sub_cost_8 / episode_count,
                       total_sub_cost_9 / episode_count, total_sub_cost_10 / episode_count,

                       total_sub_cost_11 / episode_count, total_sub_cost_12 / episode_count,
                       total_sub_cost_13 / episode_count, total_sub_cost_14 / episode_count, total_sub_cost_15 / episode_count,
                       total_sub_cost_16 / episode_count, total_sub_cost_17 / episode_count, total_sub_cost_18 / episode_count,
                       total_sub_cost_19 / episode_count, total_sub_cost_20 / episode_count
