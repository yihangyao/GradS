...
class BaseTrainer(ABC):
    ...
    def __init__(
        ...
        self.best_perf_cost = [np.inf for _ in range(len(self.cost_limit))]
        self.start_epoch = 0
        self.env_step = 0
        self.cum_cost = 0
        self.cum_episode = 0
    ...
    def __next__(self) -> Tuple[int, Dict, Dict]:
        ...
        if self.epoch % self.save_model_interval == 0 and self.epoch >= 300 :
            self.logger.save_checkpoint(suffix="_"+str(self.epoch))
        ...
        update_info.update(
            {
                ...
                # "best_cost_0": self.best_perf_cost[0],
                # "best_cost_1": self.best_perf_cost[1]
            }
        )
