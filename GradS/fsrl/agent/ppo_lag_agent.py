...
class PPOLagAgent(OnpolicyAgent):
    ...
    def __init__(
        ...
        actor = ActorProb(
            net, action_shape, max_action=max_action, unbounded=unbounded, device=device, conditioned_sigma=conditioned_sigma
        ).to(device)
        critic = [
            Critic(
                Net(state_shape, hidden_sizes=hidden_sizes, device=device),
                device=device
            ).to(device) for _ in range(self.constraint_num+1) # 2 means 1 reward + 1 cost 
        ]
        ...
        self.policy = PPOLagrangian(
            ...
            random_con_selection=self.random_con_selection,
            gradient_shaping=self.gradient_shaping,
            max_singleAC = self.max_singleAC
        )
